...
public class ParticleSystem implements SceneGraphNode, Function, Filter {
	...
	/**
	 * Gets the dimension of the phase space state (particles * 2 dimensions * 2 for
	...
	public void derivs(double t, double[] p, double[] dpdt) {
		...
		// TODO: Objective 2, for explicit integrators, compute forces, and
		// accelerations, and set dpdt
	...
	public void advanceTime(double elapsed) {
		...
		Spring.p = viscousDamping.getValue();
		...
		if (explicit.getValue()) {
			...
			int N = particles.size();
			DenseMatrix M = new DenseMatrix(2*N, 2*N);
			for(Particle particle: particles)
			{
				f.add(2*particle.index, -particle.v.x*viscousDamping.getValue());
				f.add(2*particle.index+1, -particle.v.y*viscousDamping.getValue());
				f.add(2*particle.index+1, particle.mass*gravity.getValue());
				M.add(particle.index*2, particle.index*2, particle.mass);
				M.add(particle.index*2+1, particle.index*2+1, particle.mass);
			}
			double h = elapsed;
			getVelocities(xdot);
